import pygame
from pygame import Surface

import references
from entities.entity import Entity
from ui.text_box import TextBox
from utils.sprites import get_coordinates_with_scroll


class Player(Entity):
    def __init__(self, x: int, y: int, surface: Surface):
        super().__init__("Player", r"C:\Users\Gekota\Documents\Dev\Python\CultureTest\resources\player", x, y)
        self.current_sprite = self.sprites["bottom_idle"]
        self.current_key = "bottom_idle"
        self.bouding_box = {}
        self.ghost = False
        for key, value in self.sprites.items():
            value = value.copy()
            for _x in range(value.get_width()):
                for _y in range(value.get_height()):
                    if value.get_at((_x, _y))[3] != 0:
                        value.set_at((_x, _y), (255, 0, 0, 255))
            self.bouding_box[key] = value
        self.default_x, self.default_y = x + surface.get_width() // 2 + references.game.scroll[
            0] - self.width // 2, y + surface.get_height() // 2 + references.game.scroll[1] - self.height // 2
        self.motion_ratio = 10
        self.motions = {"left": 1, "right": 2, "top": 3, "bottom": 4}
        self.life = 3
        self.orientation = 2
        self.velocity = 2

    def draw(self, surface: Surface):
        if not self.ghost:
            from utils.sprites import draw_with_scroll
            shadow: Surface = self.sprites["shadow"]
            draw_with_scroll(surface, shadow, self.x + self.width // 2 - shadow.get_width() // 2, self.y + self.height - 20)
        super().draw(surface)

    def move(self, x: int, y: int) -> None:
        forward = False
        yforward = False
        backward = False
        current_screen = references.game.screen
        x_offset = -current_screen.x
        y_offset = -current_screen.y
        if not self.ghost:
            if x > 0:
                self.orientation = 1
                s = Surface((self.width // 2, self.height // 2))
                s.blit(self.current_sprite, (-self.width//2, -self.height // 2))
                p_mask = pygame.mask.from_surface(s)
                mask_offset: tuple[float, float] = (
                    self.x + x_offset + x + self.width // 2,
                    self.y + y_offset + self.height // 2
                )
                if current_screen.masks["collisions"].overlap(p_mask, mask_offset):
                    return
                if current_screen.masks["forward"].overlap(p_mask, mask_offset):
                    forward = True
                if current_screen.masks["backward"].overlap(p_mask, mask_offset):
                    backward = True
                if current_screen.masks["yforward"].overlap(p_mask, mask_offset):
                    yforward = True
            elif x < 0:
                self.orientation = 3
                s = Surface((self.width//2, self.height//2))
                s.blit(self.current_sprite, (0, -self.height//2))
                p_mask = pygame.mask.from_surface(s)
                mask_offset: tuple[float, float] = (
                    self.x + x_offset + x,
                    self.y + y_offset + self.height // 2
                )
                if current_screen.masks["collisions"].overlap(p_mask, mask_offset):
                    return
                if current_screen.masks["forward"].overlap(p_mask, mask_offset):
                    forward = True
                if current_screen.masks["backward"].overlap(p_mask, mask_offset):
                    backward = True
                if current_screen.masks["yforward"].overlap(p_mask, mask_offset):
                    yforward = True
            if y > 0:

                self.orientation = 2
                s = Surface((self.width, self.height//2))
                s.blit(self.current_sprite, (0, -self.height//2))
                p_mask = pygame.mask.from_surface(s)
                mask_offset: tuple[float, float] = (
                    self.x + x_offset,
                    self.y + y_offset + y + self.height // 2
                )
                if current_screen.masks["collisions"].overlap(p_mask, mask_offset):
                    return
                if current_screen.masks["forward"].overlap(p_mask, mask_offset):
                    forward = True
                if current_screen.masks["backward"].overlap(p_mask, mask_offset):
                    backward = True
                if current_screen.masks["yforward"].overlap(p_mask, mask_offset):
                    yforward = True

                if self.motions["bottom"] > 8 * self.motion_ratio:
                    self.motions["bottom"] = 0
                self.motions["bottom"] += 1
                index_ = round(self.motions["bottom"] // self.motion_ratio)
                if index_ == 0:
                    index_ = 1
                self.current_sprite = self.sprites[f"bottom_{index_}"]
                self.current_key = f"bottom_{index_}"
            elif y < 0:
                self.orientation = 0
                s = Surface((self.width, self.height // 2))
                s.blit(self.current_sprite, (0, -self.height // 2))
                p_mask = pygame.mask.from_surface(s)
                mask_offset: tuple[float, float] = (
                    self.x + x_offset,
                    self.y + y_offset + y + self.height // 2
                )
                if current_screen.masks["collisions"].overlap(p_mask, mask_offset):
                    return
                if current_screen.masks["forward"].overlap(p_mask, mask_offset):
                    forward = True
                if current_screen.masks["backward"].overlap(p_mask, mask_offset):
                    backward = True
                if current_screen.masks["yforward"].overlap(p_mask, mask_offset):
                    yforward = True
        self.x += x
        self.y += y
        self.compute_scroll(x, y, current_screen.border)

        if forward or yforward:
            if references.game.question is not None:
                references.game.question.past = True
                references.game.question = None
                if yforward:
                    self.life -= 1
                    if self.life == 0:
                        references.game.game_over()
                else:
                    references.game.good_answers += 1
            custom = references.game.screen.custom
            addons = references.game.screen.addons.copy()
            references.game.next_screen()
            px, py = references.game.screen.px, references.game.screen.py
            self.x = px
            self.y = py
            if custom is not None and px == 0 and py == 0:
                index_ = 0
                if len(addons) > 0:
                    index_ = int(addons[-1].strip("addon")) - 1
                coos = custom[index_]
                self.x = coos[0]
                self.y = coos[1]
            references.game.scroll = [-self.x, -self.y]
            self.compute_scroll(0, 0, references.game.screen.border)
        elif backward:
            references.game.previous_screen()
            self.x = references.game.screen.px
            self.y = references.game.screen.py
            references.game.scroll = [-references.game.screen.px, -references.game.screen.py]
            self.compute_scroll(0, 0, references.game.screen.border)

    def interact(self):
        current_screen = references.game.screen
        x_offset = -current_screen.x
        y_offset = -current_screen.y
        match self.orientation:
            case 0:
                s = Surface((self.width+1, 2))
                s.fill((255, 0, 0, 255))
                m = pygame.mask.from_surface(s)
                m_offset: tuple[float, float] = (
                    self.x + x_offset,
                    self.y + y_offset - 2
                )
                if current_screen.masks["question"].overlap(m, m_offset):
                    self.read(0)
                elif current_screen.masks["answer1"].overlap(m, m_offset):
                    self.read(1)
                elif current_screen.masks["answer2"].overlap(m, m_offset):
                    self.read(2)
                elif current_screen.masks["answer3"].overlap(m, m_offset):
                    self.read(3)
            case 1:
                s = Surface((2, self.height+1))
                s.fill((255, 0, 0, 255))
                m = pygame.mask.from_surface(s)
                m_offset: tuple[float, float] = (
                    self.x + self.width + x_offset,
                    self.y + y_offset
                )
                if current_screen.masks["question"].overlap(m, m_offset):
                    self.read(0)
                elif current_screen.masks["answer1"].overlap(m, m_offset):
                    self.read(1)
                elif current_screen.masks["answer2"].overlap(m, m_offset):
                    self.read(2)
                elif current_screen.masks["answer3"].overlap(m, m_offset):
                    self.read(3)
            case 2:
                s = Surface((self.width+1, 2))
                s.fill((255, 0, 0, 255))
                m = pygame.mask.from_surface(s)
                m_offset: tuple[float, float] = (
                    self.x + x_offset,
                    self.y + self.height + y_offset
                )
                if current_screen.masks["question"].overlap(m, m_offset):
                    self.read(0)
                elif current_screen.masks["answer1"].overlap(m, m_offset):
                    self.read(1)
                elif current_screen.masks["answer2"].overlap(m, m_offset):
                    self.read(2)
                elif current_screen.masks["answer3"].overlap(m, m_offset):
                    self.read(3)
            case 3:
                s = Surface((2, self.height+1))
                s.fill((255, 0, 0, 255))
                m = pygame.mask.from_surface(s)
                m_offset: tuple[float, float] = (
                    self.x + x_offset - 2,
                    self.y + y_offset
                )
                if current_screen.masks["question"].overlap(m, m_offset):
                    self.read(0)
                elif current_screen.masks["answer1"].overlap(m, m_offset):
                    self.read(1)
                elif current_screen.masks["answer2"].overlap(m, m_offset):
                    self.read(2)
                elif current_screen.masks["answer3"].overlap(m, m_offset):
                    self.read(3)

    @classmethod
    def read(cls, type_: int) -> None:
        surface = references.client.surface
        content: str
        if type_ == 0:
            if references.game.question is None:
                content = "Exemple: Prennez le premier tronc d'arbre a gauche."
            else:
                content = references.game.question.text
        else:
            correct_answer = [key for key, value in references.game.question.answers.items() if value][0]
            answers = list(references.game.question.answers.keys())
            answers.remove(correct_answer)
            i: float = 0
            match references.game.question.correct:
                case 1:
                    i = 1
                case 2:
                    i = 0.5
                case 3:
                    i = 0
            if type_ == references.game.question.correct:
                content = correct_answer
            else:
                if i == 0.5:
                    if type_ == 1:
                        content = list(references.game.question.answers.keys())[type_]
                    else:
                        content = list(references.game.question.answers.keys())[type_ - 1]
                else:
                    content = list(references.game.question.answers.keys())[int(type_ - i)]
        references.game.menu = TextBox(content, surface)

    def compute_scroll(self, x: int, y: int, border=True) -> None:
        if not border:
            references.game.scroll[0] -= x
            references.game.scroll[1] -= y
            return
        current_screen = references.game.screen
        x_offset = -current_screen.x
        y_offset = -current_screen.y
        game_surface: Surface = references.client.surface
        gsw, gsh = game_surface.get_size()
        screen_surface: Surface = current_screen.background
        sw, sh = screen_surface.get_size()
        x__, y__ = get_coordinates_with_scroll(self.current_sprite, self.x - x, self.y - y)
        if x == 0 and y == 0:
            # Right
            end_screen: int = self.x + self.width // 2 + gsw // 2
            end_surface: int = sw - x_offset
            if end_screen >= end_surface and x__ == 0:
                references.game.scroll[0] += end_screen - end_surface
            # Left
            end_screen: int = self.x + self.width // 2 - gsw // 2
            end_surface: int = - x_offset
            if end_screen <= end_surface and x__ == 0:
                references.game.scroll[0] += end_screen - end_surface
            # Down
            end_screen: int = self.y + self.height // 2 + gsh // 2
            end_surface: int = sh - y_offset
            if end_screen >= end_surface and y__ == 0:
                references.game.scroll[1] += end_screen - end_surface
            # Up
            end_screen: int = self.y + self.height // 2 - gsh // 2
            end_surface: int = - y_offset
            if end_screen <= end_surface and y__ == 0:
                references.game.scroll[1] += end_screen - end_surface
            return
        error_margin = list(range(-self.velocity//2, self.velocity//2))
        if x > 0:
            # Go right
            end_screen: int = self.x + self.width // 2 + gsw // 2
            end_surface: int = sw - x_offset
            if end_screen > end_surface:
                end_screen = end_surface
            if end_screen < end_surface and x__ in error_margin:
                references.game.scroll[0] -= x
        if x < 0:
            # Go left
            end_screen: int = self.x + self.width // 2 - gsw // 2
            end_surface: int = - x_offset
            if end_screen < end_surface:
                end_screen = end_surface
            if end_screen > end_surface and x__ in error_margin:
                references.game.scroll[0] -= x
        if y > 0:
            # Go down
            end_screen: int = self.y + self.height // 2 + gsh // 2
            end_surface: int = sh - y_offset
            if end_screen > end_surface:
                end_screen = end_surface
            if end_screen < end_surface and y__ in error_margin:
                references.game.scroll[1] -= y
        if y < 0:
            # Go up
            end_screen: int = self.y + self.height // 2 - gsh // 2
            end_surface: int = - y_offset
            if end_screen < end_surface:
                end_screen = end_surface
            if end_screen > end_surface and y__ in error_margin:
                references.game.scroll[1] -= y
