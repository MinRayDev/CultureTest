...
class EditMenu(Menu):
    ...
    def __init__(self, parent: Optional['Menu'], question_index: int):
        ...
        self.menu_title = OutlinedText("ECRAN DE MODIFICATION DE QUESTIONS", int(112*self.ratio), int(38*self.ratio), (255, 255, 255), (0, 0, 200), int(15*self.ratio))
        ...
        self.question.content = game.questions[question_index].text
        ...
        self.results.append(Choice("Modifier", x, y + int(56*self.ratio), height, action=lambda: self.edit()))
        self.choices.append(self.results[-1])
        self.fairy = Fairy(int(100*self.ratio), self.answers[0].rectangle.y, limit=len(self.answers))
        self.good = -1
        for i, answer in enumerate(game.questions[question_index].answers):
            if game.questions[question_index].answers[answer]:
                self.good = i
                self.answers[i].outline_color = (0, 220, 0)
            self.answers[i].content = answer
        ...
        if self.question_done:
            if pygame.K_DOWN in events[pygame.KEYDOWN] and self.fairy.position < self.fairy.limit:
                ...
                if self.fairy.position < 3:
                    ...
                    answer: ChoiceText
                    for i, answer in enumerate(self.results):
                        if i == self.good:
    ...
    def edit(self) -> None:
        ...
        if self.question.content and self.answers[0].content and self.answers[1].content and self.answers[2].content and self.good != -1 and self.answers[0].content != self.answers[1].content and self.answers[0].content != self.answers[2].content and self.answers[1].content != self.answers[2].content:
            game.questions[self.question_index].text = self.question.content
            game.questions[self.question_index].answers = {}
            for i, answer in enumerate(self.answers):
                if i == self.good:
                    game.questions[self.question_index].answers[answer.content] = True
                else:
                    game.questions[self.question_index].answers[answer.content] = False
            game.questions[self.question_index].correct = self.good
            game.save_questions()
            if self.parent:
                self.parent.reload()
            self.back()
