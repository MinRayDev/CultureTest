...
class Text(Element):
    ...
    def __init__(self, text: str, x: int, y: int, color: tuple[int, int, int], height: int = 30):
        ...
        """
        self.content = text
        self.text = pygame.font.Font(Fonts.returnofganon, height).render(text, True, color)
    ...
    def change(self, text: str) -> None:
        ...
        """
        self.text = pygame.font.Font(Fonts.returnofganon, self.height).render(text, True, self.color)
